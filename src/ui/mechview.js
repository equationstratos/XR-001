...
export class MechView {
  ...
  setEnabled(on, labelsWanted = true) {
    ...
    if (on) {
      ...
      // Le train de commande s'etend sur toute la hauteur du drone : le
      // ressort part de son ancrage en tete (+70) et descend jusqu'au
      // coulisseau (-24). On vise a mi-hauteur de cet ensemble.
      this._prev.y -= 0.021;
      ...
      const d = 0.100 / Math.sin(THREE.MathUtils.degToRad(viewer.camera.fov / 2));
